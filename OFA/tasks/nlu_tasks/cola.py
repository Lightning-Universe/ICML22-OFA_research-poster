...
@register_task("cola", dataclass=COLAConfig)
class COLATask(OFATask):
    def __init__(self, cfg: COLAConfig, src_dict, tgt_dict):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = COLADataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
    ...
    def valid_step(self, sample, model, criterion):
        ...
        with torch.no_grad():
            ...
            FP = sum(
                [1 - ref_dict.get(hyp, 0) if hyp == 'yes' else 0 for ref_dict, hyp in zip(sample['ref_dict'], hyps)])
            TN = sum([ref_dict.get(hyp, 0) if hyp == 'no' else 0 for ref_dict, hyp in zip(sample['ref_dict'], hyps)])
            FN = sum(
                [1 - ref_dict.get(hyp, 0) if hyp == 'no' else 0 for ref_dict, hyp in zip(sample['ref_dict'], hyps)])
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if sum_logs("_score_cnt") > 0:
            ...
            metrics.log_scalar("_TN", sum_logs("_TN"))
            metrics.log_scalar("_FN", sum_logs("_FN"))
            metrics.log_derived("acc", compute_acc)
            metrics.log_derived("mcc", compute_mcc)
